...
const Navbar = () => {
  ...
  const solutions = [
    {
      ...
      items: [
        ...
        { title: 'Furniture Store', icon: Store, href: '/solutions/furniture-store', description: 'Marketing for furniture retailers' },
        { title: 'Restaurant', icon: Utensils, href: '/solutions/restaurant', description: 'Digital solutions for restaurants' }
      ],
      info: {
  ...
  return (
    <nav className="fixed w-full bg-white shadow-lg z-50" role="navigation">
      <div className="max-w-7xl mx-auto">
      <div className="flex justify-between items-center px-4 sm:px-6 lg:px-8 h-auto py-4">
          {/* Logo */}
          <Link to="/" className="flex items-center space-x-2" aria-label="OneTap Media Home" onClick={handleLinkClick}>
...
interface MobileNavSectionProps {
  title: string;
  items: { title: string; icon: React.ComponentType<any>; href: string; description: string }[];
  onLinkClick: () => void;
}

const MobileNavSection = ({ title, items, onLinkClick }: MobileNavSectionProps) => {
  const [isOpen, setIsOpen] = useState(false);
